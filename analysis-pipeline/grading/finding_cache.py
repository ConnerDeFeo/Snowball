...
# Identifies which filing block this sub-agent call is about.
...
# Identifies which category/section/prompt/model version produced (or would
# produce) the findings, so a rubric change, prompt edit, or model swap never
# serves stale cached findings.
def _version_key(rubric_category: RubricCategory, section: Section) -> str:
    ...
    return f"{rubric_category.value}#{section.value}#{sub_agent_prompt_version(direction)}#{bedrock.MODEL_ID}"
