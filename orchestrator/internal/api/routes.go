...
func NewRouter(apc *analysispipeline.Client, rpc *reviewpipeline.Client) *http.ServeMux {
	...
	mux.HandleFunc("GET /rubric", apc.HandleRubric)
	mux.HandleFunc("GET /rubric/{rubric_category}", apc.HandleRubric)
	...
	// Read-only grade queries (plain JSON, not SSE)
	mux.HandleFunc("GET /grades/{tckr}", apc.HandleGrades)
	mux.HandleFunc("GET /companies", apc.HandleGrades)
